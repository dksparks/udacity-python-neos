"""Provide filters for querying approaches and limiting results.
...
module), a reference value, and a class method `get` that subclasses can
...
class AttributeFilter:
    ...
    and calling the filter (with __call__) executes
    ...
    def __init__(self, op, value):
        """Construct an `AttributeFilter` from a predicate and value.
        ...
        :param op:
            A 2-argument predicate comparator (such as `operator.le`).
    ...
    def __str__(self):
        """Return `str(self).`"""
        return f'{self.__class__.__name__}: {self.op.__name__} {self.value}'
...
class HazardousFilter(AttributeFilter):
    """A subclass to filter by whether hazardous or not."""
    @classmethod
    def get(cls, approach):
...
def create_filters(
    ...
    :param hazardous:
        Whether the NEO of a matching `CloseApproach` is potentially
        hazardous.
